        ...
        return function (session, args, next) {
            ...
            //This is for the button functions in adaptive cards
                ...
                if(filterobj1.opt === "Show more items")
                {
                    ...
                    adaptivecards();
                    /*
                    //  This is giving only repeated results
                    getPageFunc(pageNumber, settings.pageSize).then(function (pageResult) {
                        // save current page number
                        console.log(session.dialogData.pageNumber)
                        session.dialogData.pageNumber = pageNumber;
                        console.log("This is the value of the variable pageNumber " + pageNumber)
                        ...
                        var message = new builder.Message(session)
                            ...
                            .attachments(cards);
                        //This is sending the results 
                        session.send(message);
                        
                        console.log("The new property of data x is " + filterobj1.x)

                        // more items link
                        if (filterobj1.opt === "Show more results" && pageResult.totalCount > pageNumber * settings.pageSize) 
                        {   
                            ...
                            //This is sending new values
                            session.send("This is an extra card")
                            session.send(new builder.Message(session).addAttachment(moreCard));
                            // console.log("I clicked Yes, please")
                        }
                        
                        //Do not disturb them. This is for showing adaptive cards when clicked on show more option.
                ...
                else if(filterobj1.opt === "Apply Filters")
                {
                    ...
                    getQuery()
                    setTimeout(searchsolr,3000)
                    setTimeout(adaptivecards,5000)
